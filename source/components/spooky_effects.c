...
void spooky_effects_on_select_cam(enum RoomNames room_name) {
    ...
    switch (room_name) {
        case ROOM_STAGE:
            ...
            break;
        case ROOM_DINING:
            room_effects[room_name] = AnimatronicChica.room_num == room_name
                                          ? AnimatronicChica.close_far
                                          : AnimatronicBonnie.room_num == room_name
                                          ? AnimatronicBonnie.close_far
                                          : 0; // TODO remove direct call to BONNIE and CHICA
            break;
        case ROOM_PIRATE:
            room_effects[room_name] = AnimatronicFoxy.room_num == room_name ? AnimatronicFoxy.phase : roll_pirate_empty;
            // TODO remove direct call to FOXY
            break;
        case ROOM_WEST:
            break;
        case ROOM_WEST_CORNER:
            occupants = Animatronics.get_room_occupants(room_name);
            room_effects[room_name] = (AnimatronicGoldenFreddy.room_num == ROOM_WEST_CORNER || AnimatronicGoldenFreddy.
                room_num == ROOM_OFFICE) && occupants == OCCUPANTS_EMPTY;
        ...
        case ROOM_EAST:
            occupants = Animatronics.get_room_occupants(room_name);
            room_effects[room_name] = occupants == OCCUPANTS_EMPTY
                                          ? roll_east_empty
                                          : AnimatronicChica.room_num == room_name
                                          ? AnimatronicChica.close_far
                                          : 0;
            break;
        ...
        case ROOM_BACKSTAGE:
            occupants = Animatronics.get_room_occupants(room_name);
            room_effects[room_name] = occupants == OCCUPANTS_EMPTY
                                          ? roll_backstage_empty
                                          : occupants == OCCUPANTS_ONLY_BONNIE
                                          ? roll_backstage_bonnie
                                          : 0;
            break;
        case ROOM_KITCHEN:
            break;
        case ROOM_RESTROOMS:
            room_effects[room_name] = AnimatronicChica.room_num == room_name ? AnimatronicChica.close_far : 0;
            // TODO remove direct call to CHICA
            break;
        case ROOM_OFFICE:
            break;
        case ROOM_LEFT_DOOR:
